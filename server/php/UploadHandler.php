...
class UploadHandler
{
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
            $file = new stdClass();
            $file->name = $file_name;
            $_file_path = $this->options['upload_dir'].'/'.$file_name;
            if(file_exists($_file_path)){
				$file->last = filemtime($_file_path);
            }
...
//        if($this->endWith($name,".zip")){
//        	$zip_in_dir = dirname($file_path)."/";
//        	$this->deldir($zip_in_dir."/",array(basename($file_path)));
//        	file_put_contents("./info.txt","");
//        	$zip = new ZipArchive();
//			if ($zip->open($file_path) === TRUE) {
//			    $zip->extractTo($zip_in_dir);
//			    $zip->close();
//			    unlink($file_path);
//			} else {
//			}
//        }
		    $_file_path = $this->options['upload_dir'].'/'.$name;
            if(file_exists($_file_path)){
				$file->last = filemtime($_file_path);
            }
        return $file;
     ...
     * 初始化上传
     */
    protected function init_upload_version($name){
    	   		//上传的文件名
		$filename = $name;
		$file = $this->options['upload_dir'].'/'.$filename;;
		//已经存在该文件名
		if(file_exists($file)){
			@unlink($file);
		}
		//版本索引文件
		$version_file = $this->options['upload_dir'].'/version.json';
		if(!@file_exists($version_file)){
			...
			if($version_content==""){
				...
				$version_content = json_decode($version_content,true);
			}
			
			if(!in_array($filename,$version_content)){
				array_unshift($version_content,$filename);
			}
				
//			if($this->startswith($filename,"apk")){//安卓文件
//				$data = $version_content->apk;
//			
//				$version_content->apk = $data;
//			}elseif($this->startswith($filename,"ipa")){//苹果文件
//				$data = $version_content->ipa;
//				if(!in_array($filename,$data)){
//					array_unshift($data,$filename);
//				}
//				$version_content->ipa = $data;
//			}
			file_put_contents($version_file,$this->indent(json_encode($version_content)));	
    }
...
protected function getFileList($directory) {        
    ...
    return $files;        
    	 ...
    	 */
	protected function deldir($dir,$filter=array(),$drop_owner=false) {
	  //先删除目录下的文件：
		  $dh=opendir($dir);
	...
	/**
	 * 匹配开始
	 * @param bool $cases_ensitive 大小写是否敏感.
	 */
	protected function startswith($src,$match,$cases_ensitive=true){
	 ...
	 * @param bool $cases_ensitive 大小写是否敏感.
	 */
	protected function endWith($src,$match,$cases_ensitive=true){
		if($cases_ensitive){//大小写敏感型
